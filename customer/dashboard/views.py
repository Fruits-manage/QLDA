...
# from inventory.models import InventoryStock  # Customer không cần quản lý kho
...
class DashboardHomeView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['total_customers'] = CustomerProfile.objects.filter(is_verified=True).count()
        ...
        # Customer không cần quản lý tồn kho - comment out
        ...
        context['pending_payments'] = Payment.objects.filter(
            status='pending'
        ).select_related('order', 'customer')[:10]
