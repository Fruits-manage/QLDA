...
from customer_portal.models import CustomerProfile  # Thay Company bằng CustomerProfile
import csv
import openpyxl
...
# Customer Management Views (replacing Company views)
class CustomerListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['suppliers_count'] = Company.objects.filter(company_type='supplier').count()
        context['customers_count'] = Company.objects.filter(company_type='customer').count()
        context['partners_count'] = Company.objects.filter(company_type='partner').count()
...
class CompanyCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = [
        'name', 'company_type', 'tax_code', 'email', 'phone', 'website',
...
class CompanyDetailView(LoginRequiredMixin, DetailView):
    """Chi tiết công ty"""
    model = Company
    template_name = 'management/company_detail.html'
...
class CompanyUpdateView(LoginRequiredMixin, UpdateView):
    """Cập nhật công ty"""
    model = Company
    template_name = 'management/company_edit.html'
    fields = [
        'name', 'company_type', 'tax_code', 'email', 'phone', 'website',
        'address', 'contact_person', 'contact_phone', 'contact_email', 
        'bank_name', 'bank_account', 'is_active'
...
class CustomerListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Tổng số đơn hàng từ tất cả khách hàng
        from orders.models import Order
        context['total_orders'] = Order.objects.count()
        
        # Tổng doanh thu từ tất cả đơn hàng
        from django.db.models import Sum
        context['total_revenue'] = Order.objects.filter(status='completed').aggregate(
            total=Sum('total_amount')
        )['total'] or 0
        
        # Danh sách các thành phố lớn để lọc
        context['cities'] = [
...
def export_data(request, data_type):
    ...
    if data_type == 'employees':
        ...
        # Headers
        headers = ['Tên công ty', 'Mã số thuế', 'Loại', 'Email', 'Điện thoại', 'Địa chỉ', 'Trạng thái']
        ws.append(headers)
        ...
        for company in Company.objects.all():
            ws.append([
                ...
                company.phone or '',
                company.address or '',
                'Hoạt động' if company.is_active else 'Tạm dừng'
            ])
