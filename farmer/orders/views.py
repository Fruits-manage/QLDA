...
class OrderListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        queryset = Order.objects.select_related('farmer', 'created_by').all()
...
class OrderCreateView(LoginRequiredMixin, CreateView):
    ...
    fields = ['order_type', 'farmer', 'delivery_date', 'shipping_address', 'notes', 'payment_status']
    success_url = reverse_lazy('orders:list')
    
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['companies'] = FarmerProfile.objects.filter(is_active=True)
...
class OrderUpdateView(LoginRequiredMixin, UpdateView):
    ...
    fields = ['farmer', 'order_type', 'delivery_date', 'shipping_address', 'shipping_contact', 'shipping_phone', 'notes', 'status', 'payment_status']
    context_object_name = 'order'
...
class OrderAPICreateView(View):
    ...
    def post(self, request):
        try:
            ...
            required_fields = ['farmer_id', 'order_type', 'delivery_date', 'order_details']
            ...
            try:
                ...
                return JsonResponse({
                    'success': False,
                    'error': f'FarmerProfile with ID {data["farmer_id"]} not found'
                }, status=400)
            ...
            with transaction.atomic():
                order = Order.objects.create(
                    ...
                    shipping_address=data.get('shipping_address', farmer.address),
            ...
            response_data = {
                ...
                'order': {
                    ...
                    'farmer': {
                        ...
                        'tax_code': order.farmer.tax_code
                    },
